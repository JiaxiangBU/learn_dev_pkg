...
output: 
    bookdown::gitbook:
        ...
        split_bib: TRUE
        df_print: paged
bibliography: [refs/add.bib,../imp_rmd/add.bib]
---
...
# 去除 global variables 的问题

一般 check 时会出现这样的问题，这是使用 `.` 写 pipline 产生的。
...
> Quoted arguments in tidyverse functions can trigger an **R CMD check NOTE** about undefined global variables. To avoid this: 1.Import `rlang::.data` to your package,  perhaps with the roxygen2 tag `@importFrom rlang .data` 2.Use the `.data$` pronoun in front of variable names in tidyverse[@tidyeval-2]

(ref:global-var-plot) 防止建立包时，检测出现 notes，可以让`.data$var`尽量在`aes()`和`vars`里面[@Dunnington2020]

```{r global-var-plot, fig.cap='(ref:global-var-plot)', out.width=c('50%','50%'), fig.show='hold'}
...
类似于这种情况中，显然`write_excel_csv`是一个未加上报名的函数。
加完以后记得加上 `@import`


# todos 进 commit

```{r,child="analysis/save-todos.Rmd"}
```
...
1. `usethis::use_cran_comments()`
1. `usethis::use_build_ignore("cran-comments.md")`
...
1. 增加`usethis::use_spell_check()`，增加`test`
1. 在 R 加入 `@examples`，新建 vignette，加入
1. 加入 `README.Rmd`，用 child
...
这里 `importFrom("stats", "optimise")` 是的确的。

# pending a manual inspection
...
`doi:10.1002/dir.4000060408` 就是我们需要的，格式为`<doi:10.prefix/suffix>`
...
在对应的 vignette 中 yaml 写着`%\VignetteIndexEntry{Vignette Title}`，没有自定义导致的。
...
```r
Please single quote softare names such as 'Python' in the Description field
```
...
# mis-spelling arguement

参考 https://github.com/robbriers/biotic/blob/master/cran-comments.Rmd
可以写
...
add2pkg: 
A Assistant Package for Package Development in R
...
这只是针对 `@export` 的(`regarding exported methods`)

加入`@return`，参考 http://r-pkgs.had.co.nz/man.html
如果为空，写入 `@return None`
...
# warning example doesn't exist

参考 https://support.bioconductor.org/p/102575/
...
Please do not use title case in the description.
Also please add more detail to the description.
```
...
参考 https://stackoverflow.com/questions/13463103/inst-and-extdata-folders-in-r-packaging
例如 add2ggplot 包，这样写
...
那些使用 addin 的，把`@export`删除。
没有准备好的函数，先不要 export，平时`:::`调用就好了。
...
我之前提问过这个问题
https://community.rstudio.com/t/use-check-and-get-error-about-ex-r/18263/4
...
相关问题
https://community.rstudio.com/t/r-devel-r-cmd-check-failing-because-of-time-unable-to-verify-current-time/25589/3
https://stat.ethz.ch/pipermail/r-package-devel/2019q1/003577.html


# 打开 GitHub Pages

```{r,child="../test-gh/analysis/test-github-pages.Rmd"}
```
...
# 发布 R CRAN 后续 workflow

1. 发布博客
    把 md 复制粘贴到正文，并且加上 R CRAN 的链接。
1. 比如更新 README 模板，加入 R CRAN 的安装代码
1. 如果是 Python 包，通过 setting 更新 desc
    
    ```r
    add2pkg:::setting2desc()
    ```

1. Update NEWS \@ref(news)
1. release 产生 DOI 和 Citation 更新 R CRAN
    
    ```r
    usethis::use_github_release()
    ```
    
    不用慌，可以再修改的，只会生成这样一个 untagged 的链接
    https://github.com/JiaxiangBU/rawKS/releases/tag/untagged-5dcfe009af815378d0b1
    确保 zenodo 打开。
...
1. `submit_cran()` now returns a more informative error when the CRAN submission portal is down
1. `release()` now works if the package root is not in the working directory.

# 变量参数加上`vars`

(ref:var-arg-plot) 这样可以避免输入是文本时，需要调用 `eval` 和 `parse`写表达式 [@Dunnington2020]

```{r var-arg-plot, fig.cap='(ref:var-arg-plot)', out.width=c('33%','33%','33%'), fig.show='hold'}
...
browseVignettes(package = "rhub")
```

# vignette 和 suggest

> that packages used in the vignette need to be listed in DESCRIPTION (under Suggests if they’re only used in the vignette [@Salmon2020]
